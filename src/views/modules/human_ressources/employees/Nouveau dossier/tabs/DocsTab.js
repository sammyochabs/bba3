...
const DocsTab = ({ handleInput, handleFile, data }) => {
  ...
  const handleSubmit = (e) => {
    ...
    formData.append("File", data.employee_document_file);
      ...
      <CCard>
        ...
        <CCardBody>
          <CDataTable
            ...
            scopedSlots={{
              FileName: (item) => {
                <CLink onClick={()=>{getFile("documentID",item.DocID, item.FileName, "EmployeeDocument/GetDoc")}}>{item.FileName}</CLink>
              },
              action: (item) => (
                <td>
                  <CButton
